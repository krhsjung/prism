...
- **자격증명은 앱과 같은 값이어야 한다.** `COTURN_USER`/`COTURN_PASSWORD`는 서버가 읽는
  `PRISM_TURN_USERNAME`/`PRISM_TURN_PASSWORD`와 짝이다 — 소켓이 통화 수락과 함께 내려주는
  `iceServers`가 이 자격증명으로 만들어진다. 어긋나면 클라이언트는 401을 받고 조용히
  릴레이 없이 붙는다(그래서 아래 점검 스크립트가 있다).
...
> **자격증명 회전은 아직 수동이다.** 지금은 정적 장기 자격증명 한 쌍이고, 바꾸려면 env를
> 고쳐 coturn과 서버를 함께 재기동해야 한다. 시간 제한 자격증명(coturn `use-auth-secret` +
> 서버가 매 통화마다 HMAC 발급)으로 옮기는 것이 다음 단계다 —
> [plan/webrtc.md](../plan/webrtc.md) §10.
