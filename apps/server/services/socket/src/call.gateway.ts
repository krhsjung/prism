...
// 깨운 적 있는 세션의 기억이 이만큼 쌓이면 창이 지난 항목을 걷어낸다. 항목은 그 세션을
// 다시 깨울 때만 정리되므로, 한 번 깨우고 사라진 세션은 그냥 두면 프로세스 수명만큼 남는다.
const MAX_TRACKED_WAKES = 1_000;
...
export class CallGateway implements OnModuleDestroy {
  ...
  private allowWake(sessionId: string): boolean {
    ...
    recent.push(now);
    this.wakes.set(sessionId, recent);
    if (this.wakes.size > MAX_TRACKED_WAKES) this.pruneWakes(now);
    return true;
  }

  // 창이 통째로 지난 세션의 기억을 버린다.
  private pruneWakes(now: number): void {
    for (const [sessionId, times] of this.wakes) {
      if (times.every((at) => now - at >= WAKE_WINDOW_MS)) {
  ...
  //
  // 단, **죽은 토큰은 세션에서 뗀다.** 두면 목록이 계속 `Will notify`를 그리고 다음
  // 통화도 같은 토큰을 겨눈다 — 그 기기의 사람은 "껐다 켜라"는 신호를 받을 길이 없다.
  // 지금도 그 토큰일 때만 뗀다(그사이 새로 등록했을 수 있다). 목록이 바뀌었으니 남은
  // 기기에 알린다 — 이 변화는 스윕이 잡지 못한다(구성원이 그대로라 스냅샷이 같다).
  private async wakeDevice(
    ...
    this.logger.warn(`call wake ${outcome}`);
    if (outcome !== 'rejected') return;
    try {
      const cleared = await this.sessions.clearPushTokenIfMatches(
        call.userId,
        call.callee.sessionId,
        wake.target.token,
      );
      if (cleared) {
        this.registry.broadcast(call.userId, { type: 'sessionsChanged' });
      }
    } catch (error) {
      // 던지고 잊는 자리다 — 여기서 새면 처리되지 않은 거부가 된다. 못 뗀 토큰은
      // 다음 거부에서 다시 시도된다.
      this.logger.warn(`failed to clear rejected token: ${String(error)}`);
    }
  }
  ...
  private async start(connection: SocketConnection, to: string): Promise<void> {
    ...
    try {
      ...
      const ringBefore = this.registry.connectionsOfSession(
        connection.userId,
        callee.id,
      );
      ...
      if (!this.registry.has(connection)) return;
      // **받는 쪽도 다시 본다.** 조회하는 사이에 그 기기가 소켓을 붙였을 수 있다 —
      // 그때 `opened()`는 아직 통화가 없어 배달할 것을 찾지 못했고, 여기서 옛 스냅샷대로
      // 푸시 경로로 가면 열려 있는 앱이 벨 대신 알림을 기다린다(토큰이 늦거나 죽었으면
      // 통화를 통째로 놓친다). 붙어 있으면 소켓이 이긴다.
