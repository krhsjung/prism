...
- **갱신은 필요할 때만 돕니다** — 타이머로 미리 돌지 않습니다. 요청이 401을 만나면
  한 번만 회전하고 그 요청을 재시도합니다(`src/lib/api.ts`의 `fetchWithRefresh`).
  타이머로 돌던 시절에는 **열어 두기만 한 탭이 스스로 세션을 연장**했는데, 그건 유휴
  만료를 없애는 것과 같습니다.
- **활동 표식** — 사용자가 시킨 요청에만 `X-Prism-Activity: 1`을 답니다. 서버는 이 표식이
  붙은 요청에만 유휴 창을 밀고, 회전 자체로는 밀지 않습니다. 소켓이 시킨 목록 재조회만
  표식을 빼므로(`api.sessions(background)`), 다른 기기가 붙었다 끊긴 것 때문에 내 세션이
  연장되지 않습니다.
...
- **실시간으로 맞춰집니다.** 로그인하면 `wss://…/socket`에 붙고(`src/lib/socket.ts`),
  서버가 `sessionsChanged`를 보내면 목록을 다시 가져옵니다 — 다른 기기의 로그인·로그아웃이
  새로고침 없이 반영되고, "연결됨" 배지도 그 연결로 판단합니다. 소켓이 붙어 있지 않으면
  배지를 아예 그리지 않습니다: 서버가 준 presence가 "아무도 없다"인지 "소켓 서비스가
  죽었다"인지 구분할 수 없고, 후자를 전자로 읽으면 멀쩡한 기기를 전부 "비활성"이라고
  지어내게 됩니다.
- 이 재조회는 **활동으로 세지 않습니다**(위 활동 표식) — 화면을 열어 둔 것만으로 세션이
  연장되면 유휴 만료가 무의미해집니다.
...
**가짜는 진짜 계약을 보존하거나, 타입으로 만족시킨다.** 둘 중 하나는 반드시 한다 —
어느 쪽도 아니면 화면이 나중에 쓰기 시작한 메서드가 **조용히 `undefined`가 된다.**
그 호출은 대부분 `try` 안에서 일어나므로(목록 조회 · 로그아웃 · 소켓의 회전) TypeError가
catch로 들어가 "네트워크 실패"와 구별되지 않고, **테스트는 초록으로 남는다.**

실제로 두 번 났다. `rememberPushWanted`가 목에서 빠졌을 때는 그 뒤 줄들이 통째로 건너뛰어진
채 오래 지나갔고, `pushSupported`가 빠졌을 때는 렌더가 바로 터져 곧 드러났다 — **같은
원인인데 삼키는 catch가 있느냐로 발견까지의 시간이 갈렸다.**

- `api`는 **완전한 가짜 한 벌**(`lib/test-api.ts`)을 쓴다. `satisfies`로 검사되므로 메서드가
  늘면 **그 파일 하나가** 컴파일에서 깨진다(목을 쓰는 일곱 곳이 아니라). 전부 `vi.fn()`이라
  스텁하지 않은 메서드가 진짜 `fetch`로 새지도 않는다.
- 모듈을 목할 때는 `vi.importActual`로 **원본을 펼친 위에 덮고**, 반환값에 `satisfies`를
  건다. 펼치기가 누락을 막고, `satisfies`가 덮어쓴 것의 시그니처를 막는다(실제로 목이
  `PushPermission` 대신 `string`을 돌려주던 것을 이 검사가 잡았다).
- 화면 컨텍스트 값은 `test-theme.ts`·`test-i18n.ts`처럼 **프로덕션 타입을 붙여** 만든다.
- **성공 테스트는 결말을 단언한다.** "불렀다"가 아니라 화면이 어떻게 바뀌었는지를 본다 —
  호출만 보면 그 뒤가 통째로 죽어도 통과한다(위의 첫 사고가 정확히 그 모양이었다).

⚠️ **타입은 모양을 지키지 행동을 지키지 않는다.** 네이티브도 같다 — iOS 프로토콜·Android
인터페이스가 멤버 누락을 컴파일에서 잡지만, **기본 구현이 있는 멤버는 예외다**
(`SessionsServicing.sessions(accessToken:background:)`가 그렇다).
