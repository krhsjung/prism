...
final class AppDelegate: NSObject, UIApplicationDelegate {
    func application(
        ...
        PushCategories.register(localization: ServiceContainer.shared.localization)
        Messaging.messaging().delegate = self
        // 알림 권한은 **푸시 화면에서** 묻는다(plan/push.md §5-2) — 여기서는 이미
    ...
    func application(
        ...
        ServiceContainer.shared.pushTokens.apnsRegistrationSettled()
    }
    ...
    func application(
        ...
        // 기다리던 토큰 요청이 상한까지 기다리지 않게 결말을 넘긴다.
        ServiceContainer.shared.pushTokens.apnsRegistrationSettled()
    }
}
...
extension AppDelegate: UNUserNotificationCenterDelegate {
    ...
    func userNotificationCenter(
        ...
        await PushLinks.shared.open(
            info[PushDataKey.link] as? String,
            kind: info[PushDataKey.kind] as? String
        )
    }
}
...
extension AppDelegate: MessagingDelegate {
    ...
    ///
    /// 사실만 넘긴다 — 붙이는 일은 세션의 코디네이터(`PushRegistration`)가 한다. 로그인해
    /// 있지 않으면 그쪽이 아무것도 하지 않고, 다음 로그인의 되살리기가 새 값을 붙인다.
    func messaging(_ messaging: Messaging, didReceiveRegistrationToken fcmToken: String?) {
        // 토큰 값은 로그에 남기지 않는다 — 설치 단위 식별자이고 로그는 기기에 남는다.
        Log.ui("fcm registration token updated")
        Task { @MainActor in ServiceContainer.shared.pushRegistration.tokenRotated() }
    }
}
