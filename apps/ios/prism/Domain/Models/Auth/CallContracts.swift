...
/// 잠금화면은 제목을 한 줄로 자른다.
let MAX_PUSH_TITLE_LENGTH = 60

/// 사람이 적은 것 전부(제목·문구·이미지·링크)의 **UTF-8 바이트 합**의 상한(서버 계약의
/// `MAX_PUSH_CONTENT_BYTES`). FCM의 메시지 상한(4 KB)에 문구·주소를 서너 번 싣고도 들어가게 —
/// 넘기면 서버가 400으로 접으므로 화면이 먼저 막는다. 서버 스펙이 원천과 대조한다.
let MAX_PUSH_CONTENT_BYTES = 1024
