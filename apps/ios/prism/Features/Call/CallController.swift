...
final class CallController {
    ...
    ///
    /// **물었으면 true**(이미 답이 있는 경우 포함). 소켓이 준비된 듯 보여도 그 사이 닫혔으면
    /// 보내지 못한다 — 그때는 false라, 부르는 쪽이 요청을 남겨 두었다가 다시 붙은 뒤 다시 묻는다.
    func resumeCall(_ callId: String) async -> Bool {
        guard call == nil, incoming == nil else { return true }
        // 같은 통화를 겹쳐 묻지 않는 것은 부르는 쪽(`PushCallResumer`)이 한다 — 여기는 한 번 묻는다.
        // 보내기가 끝날 때까지 기다린다 — 넘긴 순간의 true는 그 뒤의 실패를 모른다.
        let message = CallClientMessage.resume(callId: callId)
        let ok = await socket.sendAwaiting(message)
        if ok {
            log = SignalLog.append(log, SignalLog.entry(
                .sent,
                type: message.logType,
