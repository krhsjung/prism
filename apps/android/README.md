...
`AuthManager`가 수명의 **75% 지점**(하한 30초)에서 미리 회전한다 — 웹과 같은 값이다.
수명은 서버가 `accessTokenTtlMs`로 알려 준다(앱은 토큰을 열어 보지 않는다). 로그인·복원·
회전 응답 모두에 실려 오므로 로그인 직후부터 스케줄이 걸린다.

- 예약된 회전은 **반응형과 같은 문**(`refreshForRetry`)을 쓴다. 그사이 401이 먼저 갱신했다면
  아무 요청도 나가지 않는다.
- 로그아웃·자격증명 폐기에서는 예약을 거둔다. 남겨 두면 이미 끝난 세션을 되살리려는 요청이
  나간다.
- 백그라운드에서 타이머가 밀려도 손해가 없다 — 복귀하면 `restoreSession()`이 다시 확인하고
  새로 건다.
- 타이머 스코프는 생성자로 받는다. 테스트가 가상 시계를 꽂아 "75% 지점에 정확히 도는가"를
  결정적으로 본다.
