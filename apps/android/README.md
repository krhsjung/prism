...
`ApiClient`가 요청에 다는 표식 하나로 갈린다.

- 사용자가 시킨 요청에는 `X-Prism-Activity: 1`이 붙는다(`ACTIVITY_HEADER`). 서버는 **이 표식이
  붙은 요청만** 활동으로 보고 창을 민다.
- 소켓이 시킨 목록 재조회만 표식을 뺀다(`request(..., background = true)` ·
  `SessionsApi.list(background = true)`).
- `ON_RESUME`의 `restoreSession()`은 표식을 **단다**. 그 경로는 `/auth/refresh`에서 끝나므로
  여기서 빼면 "앱을 다시 연 것"이 활동으로 세어지지 않는다.

### 세션 소켓 — 다른 기기의 변화가 화면에 닿는다

`SessionSocket`이 로그인 동안 `wss://…/socket`에 붙어 `sessionsChanged`를 받고,
`DashboardViewModel`이 그 신호로 목록을 **배경 재조회**한다. 목록을 소켓이 직접 주는 것이
아니라 기존 HTTP 경로를 다시 타는 이유는, 회전·401 처리가 전부 그쪽에 있어서다.

- **소켓은 인증 상태를 바꾸지 않는다.** 확정 거절을 받아도 재조회 한 번을 시키고, 그 401을
  `ApiClient` → `SessionAuthority`(=`AuthManager`)가 처리한다. 끊김만으로 로그아웃하면
  오프라인이 곧 로그아웃이 된다.
- 만료는 **공유 회전**(`refreshForRetry`)을 탄다 — 직접 회전하면 1회용 리프레시가 두 번 나가
  재사용 탐지에 걸린다.
- OkHttp의 프로토콜 ping(20초)과 별개로, 서버 하트비트가 45초 없으면 죽은 것으로 보고
  다시 붙는다. 둘은 역할이 다르다: ping은 죽은 소켓을 빨리 실패시키고, 하트비트는 **우리가**
  상대의 침묵을 알아채는 수단이다.

`app/src/test/…/live/SessionLifetimeLiveTest`가 이 둘을 **실제 서버 + 실제 클라이언트 스택**
으로 확인한다(에뮬레이터 없이 JVM에서 돈다). `PRISM_LIVE_API_URL`이 없으면 건너뛴다.

```bash
PRISM_LIVE_API_URL=https://<서버> ./gradlew testDebugUnitTest --tests '*SessionLifetimeLiveTest'
```
