...
class PrismMessagingService : FirebaseMessagingService() {
     ...
     * (plan/push.md §5-2), 살아 있는 세션을 고치는 경로는 두지 않았다. 그래서 지금
     * 세션은 죽은 토큰을 들고 있고, 전송은 `rejected`로 실패한다.
     *
     * 대신 **화면이 그 사실을 말한다** — 푸시 화면이 지금 토큰과 로그인 때 보낸 토큰을
     * 대조해 "다시 로그인하세요" 한 줄을 띄운다(`PushTokens.sentToken`).
     */
