...
소켓이 그대로 나머지의 입력이 됩니다. 그래서 **소켓이 붙어 있으면 즉시 울리고, 아니면
알림으로 깨웁니다** — 소켓의 유무는 "걸 수 있는가"가 아니라 "어떻게 닿는가"를 가릅니다.
...
- **푸시 알림**: 대상을 **여럿 골라** 한 번에 보내고, 알림에 이미지·링크·버튼을 실을 수
  있습니다. 같은 설치가 여러 세션에 걸리면 토큰 기준으로 합쳐 **한 번만** 보내고, 그
  사실을 화면이 그 줄에 적습니다. 등록 토큰이 **세션 레코드 안에서만** 삽니다 — 별도 테이블도 정리 잡도
  없고, 로그아웃·폐기·유휴 만료가 그대로 토큰의 수명입니다. 목록에는 토큰이 아니라 파생
  불리언 하나(`pushRegistered`)만 나가고, 클라이언트는 "이 세션에 보내줘"라고만 하면
  서버가 레코드에서 꺼냅니다. **소켓 없는 기기의 통화도 같은 배관으로 깨웁니다** —
  알림 페이로드에는 `callId`와 건 기기 종류만 담고(알림은 잠금화면에 뜹니다), 알림을 열면
  앱이 소켓을 붙여 통화가 아직 살아 있는지 묻습니다([plan/push.md](plan/push.md)).
...
- **품질**: TypeScript `unknown`/`any` 키워드 금지(lint 강제) · strict +
  `noUncheckedIndexedAccess` · 자동 테스트 **서버 453 · 웹 219 · iOS 111 · Android 124**.
