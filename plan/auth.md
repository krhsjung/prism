...
9. **예약된 회전도 자기 세션의 것일 때만 돈다.** 선제 갱신 타이머는 예약 시점의 표식을
   들고 있다가 깨어날 때 대조한다 — 로그아웃 뒤에도 남은 타이머가 돌면 끝난 세션을
   되살리려는 요청이 나가고, 늦게 도착한 회전 응답이 로그인 화면 뒤에 다음 타이머를 심는다.

   반대 방향도 같다: **정리는 소유권을 확인한 뒤에 한다.** 앞 세션의 자격증명을 버리려다
   소유권 확인보다 타이머를 먼저 끄면, 정작 아무것도 버리지 않으면서 지금 세션의 예약만
   꺼진다 — 멀쩡히 로그인된 채로 선제 갱신만 죽어 방치된 화면이 idle 창에서 만료된다.
...
11. **로그인 화면의 안내는 아는 것만 말한다.** 스스로 로그아웃한 것이 아닌데 로그인
    ...
    > (7일)에서만 열린다.
12. **일시적 실패로 소모된 타이머는 다시 건다.** 예약 회전이 오프라인으로 한 번 실패하면
    타이머는 이미 쓰였다. 거기서 놓으면 연결이 돌아와도 선제 갱신이 **영영 멈춰**, 세션이
    idle 창에서 조용히 죽는다. 세션이 살아 있는 한 짧은 간격(60초)으로 다시 시도한다.
